...
public class Fantasyai : ITranslator
{
    private readonly Settings cfg;

    public string name => "FantasyAI";

    public Fantasyai(Settings config)
    {
        cfg = config;
    }
    ...
    public async Task<string> translate(string text, string targetlang)
    {
        try
        {
            ...
            var response = await client.PostAsync("https://fantasyai.cloud/api/v1/chat/completions", content);
